...
if len (sys.argv) < 2:
    print (("Usage: %s <target.aac>")%(sys.argv[0]))
...
while True:
    ...
    #print("hdr[0] = 0b{:b}".format(hdr[0]))
    #print("hdr[1] = 0b{:b}".format(hdr[1]))
    #print("syncword=0b{:12b}".format(syncword))
...
# parse adts_variable_header()
    ...
    if num_rawdata_blocks == 0:
        ...
        # {raw_data_block() + adts_raw_data_block_error_check()} * N
        aacfile.read(size)
    ...
    print("#{},{},{},frame length={},fullness={},num_rawdata_blocks={},0x{:04x}".format(frame_no, copyright_id_bit, copyright_id_start, frame_length, adts_buf_fullness, num_rawdata_blocks, crc_check))
